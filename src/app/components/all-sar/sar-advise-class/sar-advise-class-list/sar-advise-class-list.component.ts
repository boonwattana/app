...
import { SarAdviseClassListModel } from '../sar-advise-class-model';
import { SarAdviseClassService } from '../sar-advise-class.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-advise-class-list',
  templateUrl: './sar-advise-class-list.component.html',
  styleUrls: ['./sar-advise-class-list.component.scss']
})
export class SarAdviseClassListComponent extends BaseListComponent<SarAdviseClassListModel> implements BaseListInterface {
  tableName:string ='sar_advise_class'
  moduleName:string = 'MODULE.SAR_ADVISE_CLASS'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarAdviseClassService,
  ...
  setDataGridOption(): void {
    ...
    const columns: ColumnModel[] = [
      ...
      {
        label: 'จำนวนนักเรียนรวมทั้งสิ้น (คน)',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-advise-class',id,isView)
  }
}
