...
import { SarAnotherSpeacialDutyListModel } from '../sar-another-speacial-duty-model';
import { SarAnotherSpeacialDutyService } from '../sar-another-speacial-duty.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-another-speacial-duty-list',
  templateUrl: './sar-another-speacial-duty-list.component.html',
  styleUrls: ['./sar-another-speacial-duty-list.component.scss']
})
export class SarAnotherSpeacialDutyListComponent extends BaseListComponent<SarAnotherSpeacialDutyListModel> implements BaseListInterface {
  tableName:string ='sar_another_speacial_duty'
  moduleName:string = 'MODULE.SAR_ANOTHER_SPEACIAL_DUTY'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarAnotherSpeacialDutyService,
  ...
  setSerachCondtion(): void {
    const searchConditions:SearchCondition[] = [
      {
        tableName:'sar_another_speacial_duty',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-another-speacial-duty',id,isView)
  }
}
