...
import { SarCrudAssessmentListModel } from '../sar-crud-assessment-model';
import { SarCrudAssessmentService } from '../sar-crud-assessment.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-crud-assessment-list',
  templateUrl: './sar-crud-assessment-list.component.html',
  styleUrls: ['./sar-crud-assessment-list.component.scss']
})
export class SarCrudAssessmentListComponent extends BaseListComponent<SarCrudAssessmentListModel> implements BaseListInterface {
  tableName:string ='sar_crud_assessment'
  moduleName:string = 'MODULE.SAR_CRUD_ASSESSMENT'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarCrudAssessmentService,
  ...
  setDataGridOption(): void {
    ...
    const columns: ColumnModel[] = [
      ...
      {
        label: 'จำนวนนักเรียนที่ประเมิน (คน)',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-crud-assessment',id,isView)
  }
}
