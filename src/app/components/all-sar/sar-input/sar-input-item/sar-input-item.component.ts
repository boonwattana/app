...
import { SarInputService } from '../sar-input.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-input-item',
  templateUrl: './sar-input-item.component.html',
  styleUrls: ['./sar-input-item.component.scss']
})
export class SarInputItemComponent extends BaseItemComponent<SarInputItemModel>  implements BaseItemInterface {
  constructor(el:ElementRef,renderer:Renderer2,private readonly service:SarInputService,private userDataService:UserDataService, router: Router , route:ActivatedRoute){
  ...
  onContinue(): void {
    this.toItemSarCreate(this.validateField())
  }
  async toItemSarCreate(isValid:boolean){    
    if(isValid){
      this.model.refId= this.model.schoolyear+'-'+ this.model.teacherId;
      this.router.navigate(['sar'+'/'+ this.model.refId+'-create']);
