...
import { SarInputListModel } from '../sar-input-model';
import { SarInputService } from '../sar-input.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-input-list',
  templateUrl: './sar-input-list.component.html',
  styleUrls: ['./sar-input-list.component.scss']
})
export class SarInputListComponent extends BaseListComponent<SarInputListModel> implements BaseListInterface {
  tableName:string ='sar'
  moduleName:string = 'สร้าง รายงานผลการปฏิบัติงานและผลการประเมินตนเองรายบุคคล (Self Assessment Report: SAR)'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarInputService,
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-input',id,isView)
  }
}
