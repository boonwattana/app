...
import { SarOrderedPositionListModel } from '../sar-ordered-position-model';
import { SarOrderedPositionService } from '../sar-ordered-position.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-ordered-position-list',
  templateUrl: './sar-ordered-position-list.component.html',
  styleUrls: ['./sar-ordered-position-list.component.scss']
})
export class SarOrderedPositionListComponent extends BaseListComponent<SarOrderedPositionListModel> implements BaseListInterface {
  tableName:string ='sar_ordered_position'
  moduleName:string = 'รายงาน คำสั่งแต่งตั้งการปฏิบัติหน้าที่ '
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarOrderedPositionService,
  ...
  setSerachCondtion(): void {
    const searchConditions:SearchCondition[] = [
      {
        tableName:'sar_ordered_position',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-ordered-position',id,isView)
  }
}
