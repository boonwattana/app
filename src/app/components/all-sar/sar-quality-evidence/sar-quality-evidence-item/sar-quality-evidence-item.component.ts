...
import { SarQualityEvidenceItemModel } from '../sar-quality-evidence-model';
import { SarQualityEvidenceService } from '../sar-quality-evidence.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-quality-evidence-item',
  templateUrl: './sar-quality-evidence-item.component.html',
  styleUrls: ['./sar-quality-evidence-item.component.scss']
})
export class SarQualityEvidenceItemComponent extends BaseItemComponent<SarQualityEvidenceItemModel>  implements BaseItemInterface {
  ...
  async onSubmit(isValid:boolean){    
    if(isValid){
      if(this.isUpdateMode){
        ...
        this.model.standard_type = 1;//มาตรฐานที่ 1  คุณภาพของผู้เรียน
  ...
  backTolist(){    
    this.toList('sar-quality-evidence')
  }
}
