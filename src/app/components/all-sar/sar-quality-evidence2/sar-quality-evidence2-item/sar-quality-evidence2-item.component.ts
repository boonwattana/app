...
import { SarQualityEvidenceItemModel } from '../sar-quality-evidence2-model';
import { SarQualityEvidenceService } from '../sar-quality-evidence2.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-quality-evidence2-item',
  templateUrl: './sar-quality-evidence2-item.component.html',
  styleUrls: ['./sar-quality-evidence2-item.component.scss']
})
export class SarQualityEvidenceItemComponent extends BaseItemComponent<SarQualityEvidenceItemModel>  implements BaseItemInterface {
  ...
  async onSubmit(isValid:boolean){    
    if(isValid){
      if(this.isUpdateMode){
        ...
        this.model.standard_type = 2;//มาตรฐานที่ 2  กระบวนการบริหารและการจัดการของผู้บริหารสถานศึกษา
  ...
  backTolist(){    
    this.toList('sar-quality-evidence2')
  }
}
