...
import { SarQualityEvidenceItemModel } from '../sar-quality-evidence3-model';
import { SarQualityEvidenceService } from '../sar-quality-evidence3.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-quality-evidence3-item',
  templateUrl: './sar-quality-evidence3-item.component.html',
  styleUrls: ['./sar-quality-evidence3-item.component.scss']
})
export class SarQualityEvidenceItemComponent extends BaseItemComponent<SarQualityEvidenceItemModel>  implements BaseItemInterface {
  ...
  async onSubmit(isValid:boolean){    
    if(isValid){
      if(this.isUpdateMode){
        ...
        this.model.standard_type = 3;//มาตรฐานที่ 3  กระบวนการจัดการเรียนการสอนที่เน้นผู้เรียนเป็นสำคัญ
  ...
  backTolist(){    
    this.toList('sar-quality-evidence3')
  }
}
