...
import { SarResearchInClassListModel } from '../sar-research-in-class-model';
import { SarResearchInClassService } from '../sar-research-in-class.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-research-in-class-list',
  templateUrl: './sar-research-in-class-list.component.html',
  styleUrls: ['./sar-research-in-class-list.component.scss']
})
export class SarResearchInClassListComponent extends BaseListComponent<SarResearchInClassListModel> implements BaseListInterface {
  tableName:string ='sar_research_in_class'
  moduleName:string = 'MODULE.SAR_RESEARCH_IN_CLASS'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarResearchInClassService,
  ...
  setSerachCondtion(): void {
    ...
    {
      tableName:'sar_research_in_class',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-research-in-class',id,isView)
  }
}
