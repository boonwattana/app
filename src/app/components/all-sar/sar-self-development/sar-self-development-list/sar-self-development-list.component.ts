...
import { SarSelfDevelopmentListModel } from '../sar-self-development-model';
import { SarSelfDevelopmentService } from '../sar-self-development.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-self-development-list',
  templateUrl: './sar-self-development-list.component.html',
  styleUrls: ['./sar-self-development-list.component.scss']
})
export class SarSelfDevelopmentListComponent extends BaseListComponent<SarSelfDevelopmentListModel> implements BaseListInterface {
  tableName:string ='sar_self_development'
  moduleName:string = 'MODULE.SAR_SELF_DEVELOPMENT'
  lastRemoveId:number = undefined;
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  teacherOption:SelectItems[] = []
  constructor(private service: SarSelfDevelopmentService,
    ...
    const columns: ColumnModel[] = [
      {
        ...
        tableName: 'sar_self_development',
        visibility: true,
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-self-development',id,isView)
  }
}
