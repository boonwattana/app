...
import { SarStudentEstimateTeachingListModel } from '../sar-student-estimate-teaching-model';
import { SarStudentEstimateTeachingService } from '../sar-student-estimate-teaching.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-student-estimate-teaching-list',
  templateUrl: './sar-student-estimate-teaching-list.component.html',
  styleUrls: ['./sar-student-estimate-teaching-list.component.scss']
})
export class SarStudentEstimateTeachingListComponent extends BaseListComponent<SarStudentEstimateTeachingListModel> implements BaseListInterface {
  tableName:string ='sar_student_estimate_teaching'
  moduleName:string = 'MODULE.SAR_STUDENT_ESTIMATE_TEACHING'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarStudentEstimateTeachingService,
  ...
  setSerachCondtion(): void {
    const searchConditions:SearchCondition[] = [
      {
        tableName:'sar_student_estimate_teaching',
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-student-estimate-teaching',id,isView)
  }
}
