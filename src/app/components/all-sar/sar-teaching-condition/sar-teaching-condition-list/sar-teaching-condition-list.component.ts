...
import { SarTeachingConditionListModel } from '../sar-teaching-condition-model';
import { SarTeachingConditionService } from '../sar-teaching-condition.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-teaching-condition-list',
  templateUrl: './sar-teaching-condition-list.component.html',
  styleUrls: ['./sar-teaching-condition-list.component.scss']
})
export class SarTeachingConditionListComponent extends BaseListComponent<SarTeachingConditionListModel> implements BaseListInterface {
  tableName:string ='sar_teaching_condition'
  moduleName:string = 'MODULE.SAR_TEACHING_CONDITION'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarTeachingConditionService,
  ...
  setSerachCondtion(): void {
    const searchConditions:SearchCondition[] = [
      {
        tableName:'sar_teaching_condition',
  ...
  setDataGridOption(): void {
    ...
    const columns: ColumnModel[] = [
      ...
      {
        label: 'ตรงตามวุฒิ/สาขาวิชาที่จบการศึกษา',
        textKey: 'choice1',
        type: ColumnType.STRING,
        tableName: 'sar_teaching_condition',
        visibility: true,
      ...
      {
        label: 'ตรงกับความต้องการ/ความสนใจ',
        textKey: 'choice5',
        type: ColumnType.STRING,
        tableName: 'sar_teaching_condition',
        visibility: true,
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-teaching-condition',id,isView)
  }
}
