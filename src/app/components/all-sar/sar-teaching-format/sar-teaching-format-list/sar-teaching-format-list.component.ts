...
import { SarTeachingFormatListModel } from '../sar-teaching-format-model';
import { SarTeachingFormatService } from '../sar-teaching-format.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-teaching-format-list',
  templateUrl: './sar-teaching-format-list.component.html',
  styleUrls: ['./sar-teaching-format-list.component.scss']
})
export class SarTeachingFormatListComponent extends BaseListComponent<SarTeachingFormatListModel> implements BaseListInterface {
  tableName:string ='sar_teaching_format'
  moduleName:string = 'MODULE.SAR_TEACHING_FORMAT'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarTeachingFormatService,
  ...
  setDataGridOption(): void {
    ...
    const columns: ColumnModel[] = [
      ...
      {
        label: 'LABEL.TEACHING_FORMAT_OTHER',
        textKey: 'teachingFormatOther',
      ...
      {
        label: 'LABEL.TEACHING_FORMAT_OTHER',
        textKey: 'teachingFormatOtherNote',
        type: ColumnType.STRING,
        tableName: 'sar_teaching_format',
        visibility: true,
        sorting: SortType.NONE,
  ...
  toItemPage(id:number,isView:boolean){
    this.toItem('sar-teaching-format',id,isView)
  }
}
