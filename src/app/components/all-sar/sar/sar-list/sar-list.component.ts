...
import { SarListModel } from '../sar-model';
import { SarService } from '../sar.service';
import { UserDataService } from 'src/app/shared/services/user-data.service';
@Component({
  selector: 'sar-list',
  templateUrl: './sar-list.component.html',
  styleUrls: ['./sar-list.component.scss']
})
export class SarListComponent extends BaseListComponent<SarListModel> implements BaseListInterface {
  tableName:string ='sar'
  moduleName:string = 'รายการ รายงานผลการปฏิบัติงานและผลการประเมินตนเองรายบุคคล (Self Assessment Report: SAR)'
  lastRemoveId:number = undefined;
  teacherOption:SelectItems[] = []
  be:number =new Date().getFullYear()+543;
  schoolyear= this.be.toString();
  constructor(private service: SarService,
