...
import { ParentStatusItemModel } from '../parent-status-model';
import { ParentStatusService } from '../parent-status.service';

@Component({
  selector: 'parent-status-item',
  templateUrl: './parent-status-item.component.html',
  styleUrls: ['./parent-status-item.component.scss']
})
export class ParentStatusItemComponent extends BaseItemComponent<ParentStatusItemModel>  implements BaseItemInterface {
  constructor(el:ElementRef,renderer:Renderer2,private readonly service:ParentStatusService, router: Router , route:ActivatedRoute){
