...
@Component({
  selector: 'sdq-table-item',
  templateUrl: './sdq-table-item.component.html',
  styleUrls: ['./sdq-table-item.component.scss']
})
export class SdqTableItemComponent extends BaseItemComponent<SdqTableItemModel>  implements BaseItemInterface {
  isStudent = false
  constructor(el:ElementRef,renderer:Renderer2,private readonly service:SdqTableService, router: Router , route:ActivatedRoute,private readonly messageService:MessageService,
...
customValidate(){
      ...
      if(this.isUpdateMode){
        ...
        this.service.update(this.model.sdq_id,this.model).subscribe(value=>{
          // if(this.userDataService.isStudent()){
            ...
            this.toItem('sdq-table',+infoId,true);
        ...
        this.model.studentId = this.studentID;
        this.model.estimateType = 1; //1 นักเรียนประเมินตนเอง 2. ครูประเมินนักเรียน 3.ผู้ปกครองประเมินนักเรียน
        //this.birthDate
  ...
  backTolist(){    
    this.router.navigate(['sdq-table/'+this.studentID]); 
  }
  ...
  pageEdit(){
    const infoId = this.router.url.split('/').reverse()[0]
    this.toItem('sdq-table',+infoId,false);
  }
  getAll(){
    const fieldList = [
    'choice01'
