...
export class SdqTeacherService {
  servicePath = '/sdq-teacher';
  constructor(private gateway: GatewayService) { }
  create(model:SdqTeacherItemModel):any{    
  ...
  getItem(id:number): any {
    const url = `${this.servicePath}/itemTeacherSdq/${id}`;
    return  this.gateway.get(url);
  }
  update(id:number,model:SdqTeacherItemModel): any {
  ...
  initial(id:number):any{
    ...
    return of(new SdqTeacherItemModel())  ;
  }
  ...
  getSDQCurrentTermData(id:number):any{
    const url = `${this.servicePath}/get-sdq-current-term-data-teacher/${id}`;
    return  this.gateway.get(url);
  }
  getSumSDQTeacher(teacherId:any):any{
    
    const url = `${this.servicePath}/get-sum-sdq-teacher/${teacherId}`;
    return  this.gateway.get(url);
  }
