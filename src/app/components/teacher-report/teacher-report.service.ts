...
import { TeacherReportItemModel } from './teacher-report-model';
@Injectable({
  providedIn: 'root'
})
export class TeacherReportService {
  ...
  create(model:TeacherReportItemModel):any{    
  ...
  //   return  this.gateway.get(url);
  // }
  update(id:number,model:TeacherReportItemModel): any {
  ...
  initial():any{
    const model = new TeacherReportItemModel()
    model.countryId  =66
    return of(model)  ;
  }
