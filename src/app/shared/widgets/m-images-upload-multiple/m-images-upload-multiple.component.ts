...
@Component({
  selector: 'm-images-upload-multiple',
  templateUrl: './m-images-upload-multiple.component.html',
  styleUrls: ['./m-images-upload-multiple.component.scss']

})
export class MFileUploadMultipleComponent  extends BaseWidget implements OnInit{
  ...
  myUploader(event:any) {
      ...
      if(image.name){
        ...
        try {
          ...
          if((this.urlList.length+this.imagesList.length)<5){
            this.urlList.push(_event.target.result);
          }else{
            this.confirmationService.confirm({
              message: 'เพิ่มภาพได้สูงสุด 5 ภาพ',
              accept: () => {
  ...
  removeImage(it:any){
    ...
    this.gateway.delete(`/images/${it.id}`).subscribe(result=>{
      ...
      if(result){
        this.imagesList = this.imagesList.filter(fl=>fl.id != result.id)
      }
