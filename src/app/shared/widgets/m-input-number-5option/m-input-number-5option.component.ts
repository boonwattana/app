...
@Component({
  selector: 'm-input-number-5option',
  templateUrl: './m-input-number-5option.component.html',
  styleUrls: ['./m-input-number-5option.component.scss']
})
export class MInputNumber5OptionComponent extends BaseWidget implements OnInit {
  value1:number
  value2:number
  value3:number
  value4:number
  ...
  onInputChange5(e:any){ 
    this.model1 = false   
    this.model2 = false   
    this.model3 = false   
    this.model4 = false  
    this.model = this.value5
    if(e){
      this.setModelChange.emit(this.value5)
    }else{
      this.setModelChange.emit(null)
    }
  }
